...
export default function Profiles() {
	...
	const featured = workspace.profiles.featuredEmployee
							...
							<div className="flex items-center justify-between gap-3 xl:justify-end">
								...
								<div className="flex items-center gap-3 rounded-full bg-white px-3 py-2">
									...
									<div className="h-10 w-10 overflow-hidden rounded-full bg-[linear-gradient(135deg,#0f51ff,#7ea4ff)] ring-2 ring-[#eef3ff]">
										<img alt={manager.fullName} className="h-full w-full object-cover" src={profileImage} />
									</div>
							...
							<div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_360px]">
								<article className="overflow-hidden rounded-[26px] border border-slate-200/80 bg-white">
									<div className="overflow-x-auto">
										<table className="min-w-full text-left">
											...
											<tbody>
												{workspace.profiles.roster.map((employee, index) => (
													<tr key={employee.userId} className={`border-t border-slate-100 text-sm text-slate-700 ${index === 0 ? 'bg-[#eef2ff]' : 'bg-white'}`}>
												...
												{!workspace.profiles.roster.length && !isLoading ? (
													<tr className="border-t border-slate-100 text-sm text-slate-500">
														<td className="px-5 py-4 sm:px-6" colSpan="5">No employee records are available yet.</td>
													</tr>
												) : null}
											</tbody>
										</table>
									...
									<div className="flex items-center justify-between border-t border-slate-100 px-5 py-4 text-sm text-slate-500 sm:px-6">
										<span>{workspace.profiles.paginationLabel || (isLoading ? 'Loading roster...' : 'No pagination data')}</span>
								...
								<aside className="rounded-[26px] border border-slate-200/80 bg-white p-5 sm:p-6">
									{featured ? (
										...
										<div className="text-sm text-slate-500">{isLoading ? 'Loading employee details...' : 'No employee detail is available yet.'}</div>
									)}
								</aside>
							</div>
						</section>
