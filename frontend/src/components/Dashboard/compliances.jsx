...
function PolicyCard({ item }) {
	return (
		<article className="rounded-[26px] border border-slate-200/80 bg-white p-5 sm:p-6">
			...
			{item.checklist ? <div className="mt-6 space-y-3">{item.checklist.map((entry) => <div key={entry} className="flex items-center gap-3 rounded-2xl bg-[#f8faff] px-3 py-2.5 text-sm text-slate-600"><FiCheckCircle className="h-4 w-4 text-emerald-500" />{entry}</div>)}</div> : null}
		</article>
	)
}
...
function ActivityRow({ item }) {
	return (
		<div className="grid gap-3 rounded-2xl bg-white px-4 py-4 md:grid-cols-[auto_110px_1fr_auto_auto] md:items-center md:px-5">
			...
			<button className="rounded-xl bg-[#f1f4ff] px-3 py-2 text-sm font-bold text-slate-700">{item.action || 'View Change'}</button>
		</div>
	)
}
				...
				<div className="dashboard-main-offset flex min-h-0 flex-1 flex-col h-screen overflow-hidden">
					<header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/75 px-4 py-4 backdrop-blur-xl sm:px-6 xl:px-8">
						...
						<div className="mt-4 flex flex-col gap-4 xl:mt-0 xl:flex-row xl:items-center xl:justify-between">
							<label className="relative w-full max-w-3xl"><FiSearch className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type="search" placeholder="Search policies, violations, or audit notes..." className="h-12 w-full rounded-full border border-slate-200/80 bg-[#f5f7ff] px-11 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#0f51ff] focus:bg-white" /></label>
					...
					<div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6 lg:px-8 xl:px-10">
						<section className="space-y-6">
							<div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
								...
								<div className="flex flex-wrap gap-3"><button className="rounded-xl bg-[#e8edff] px-4 py-2.5 text-sm font-bold text-slate-700">Full Audit Log</button><button className="rounded-xl bg-[#dce6ff] px-4 py-2.5 text-sm font-bold text-[#0f51ff]">Policy Settings</button></div>
							</div>
							...
							<div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_240px_240px]">
								<article className="rounded-[26px] border border-rose-200/70 bg-[#fff1f2] p-5 sm:p-6 xl:col-span-1">
									...
									<div className="mt-5 flex flex-wrap gap-3"><button className="rounded-xl bg-[#d92d20] px-4 py-2.5 text-sm font-extrabold text-white">Address Now</button><button className="rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-[#d92d20]">View Details</button></div>
								</article>
							...
							<div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black tracking-[-0.04em] text-slate-950">Standard Labor Rules</h2><div className="flex items-center gap-3 text-slate-500"><FiFilter className="h-4 w-4" /><div className="grid h-6 w-6 grid-cols-2 gap-1"><span className="rounded-sm bg-slate-700" /><span className="rounded-sm bg-slate-700" /><span className="rounded-sm bg-slate-700" /><span className="rounded-sm bg-slate-700" /></div></div></div>
							<div className="grid gap-5 lg:grid-cols-3">{compliance.policies.map((item) => <PolicyCard key={item.title} item={item} />)}</div>
							{!compliance.policies.length && !isLoading ? <div className="rounded-2xl border border-slate-100 bg-white p-4 text-sm text-slate-500">No compliance policies are currently available.</div> : null}
							<article className="rounded-[26px] border border-slate-200/80 bg-[#eef3ff] p-5 sm:p-6">
								<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-black tracking-[-0.04em] text-slate-950">Recent Compliance Activity</h2><p className="mt-1 text-sm text-slate-500">Reviewing last 24 hours of system-flagged events.</p></div><a className="text-sm font-bold text-[#0f51ff]" href="#audit">Full Audit Log →</a></div>
								<div className="mt-5 space-y-3">{compliance.activity.map((item, index) => <ActivityRow key={`${item.label}-${index}`} item={item} />)}</div>
							</article>
						</section>
					</div>
				</div>
