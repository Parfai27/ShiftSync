...
public class ManagerWorkspaceService {
    ...
    public ManagerWorkspaceResponse getWorkspace(Long userId) {
        ...
        return new ManagerWorkspaceResponse(
            ...
            buildSchedulingSection(branchEmployees, branchShifts, assignments),
    ...
    public void decideAdjustment(Long adjustmentId, AdjustmentDecisionRequest request) {
        ...
        if (!manager.getBranch().getId().equals(adjustment.getEmployee().getBranch().getId())) {
            throw new IllegalArgumentException("Adjustment does not belong to the manager branch");
        }
    ...
    private ProfilesSection buildProfilesSection(
        ...
        long workloadHours = assignments.stream()
            .filter(item -> item.getEmployee().getId().equals(featured.getId()))
            .map(ShiftAssignment::getShift)
            .filter(shift -> !shift.getShiftDate().isBefore(LocalDate.now().with(DayOfWeek.MONDAY)))
            .mapToLong(shift -> java.time.Duration.between(shift.getStartTime(), shift.getEndTime()).toHours())
            .sum();

        EmployeeDetail detail = new EmployeeDetail(
            featured.getFullName(),
            featuredProfile != null ? featuredProfile.getJobTitle() : "Pharmacy Staff",
            featuredProfile != null ? featuredProfile.getEmployeeCode() : "N/A",
            initials(featured.getFullName()),
            featured.getEmail(),
            featuredProfile != null ? defaultString(featuredProfile.getPhoneNumber(), "Not on file") : "Not on file",
            featuredProfile != null && featuredProfile.getHireDate() != null ? featuredProfile.getHireDate().format(DATE_LABEL) : "Not recorded",
            manager.getBranch().getLocation(),
            workloadHours + "h / 40h weekly",
            expertiseForRole(featuredProfile != null ? featuredProfile.getJobTitle() : "Pharmacy Staff"),
            weeklyAvailability(assignments, featured.getId())
        );

        return new ProfilesSection(
            "Manage " + employees.size() + " pharmacy team members at " + manager.getBranch().getName() + ".",
    ...
    private SchedulingSection buildSchedulingSection(List<User> employees, List<Shift> shifts, List<ShiftAssignment> assignments) {
        List<Shift> nextWeekShifts = shifts.stream()
            .filter(shift -> !shift.getShiftDate().isBefore(LocalDate.now()))
            .limit(7)
            .toList();

        int totalHours = nextWeekShifts.stream()
            .mapToInt(shift -> (int) java.time.Duration.between(shift.getStartTime(), shift.getEndTime()).toHours() * shift.getAssignedStaff())
            .sum();
        ...
        List<SchedulingStat> stats = List.of(
            new SchedulingStat("Total Hours", String.valueOf(totalHours), "+6% vs current rota"),
            ...
            new SchedulingStat("Labor Budget", laborBudget + "%", "of planned hours")
        );

        List<SchedulingDay> days = nextWeekShifts.stream()
            .map(shift -> new SchedulingDay(
                shift.getShiftDate().format(SHORT_DAY),
                shift.getShiftDate().format(SHORT_DATE),
                shift.getAssignedStaff() < shift.getRequiredStaff() ? "GAP" : null
            ))
            .toList();
        ...
        List<ScheduleRow> rows = employees.stream()
            ...
            .map(employee -> {
                ...
                for (Shift dayShift : nextWeekShifts) {
                    List<ScheduleBlock> dayBlocks = employeeShifts.stream()
                        .filter(shift -> shift.getId().equals(dayShift.getId()))
                        .map(shift -> new ScheduleBlock(
                            formatShiftWindow(shift),
                            shift.getStartTime().getHour() < 12
                                ? "bg-blue-100 border-blue-500 text-blue-700"
                                : "bg-indigo-100 border-indigo-500 text-indigo-700"
                        ))
                        .toList();
                    blocks.add(dayBlocks);
                }
        ...
        List<LegendItem> legend = List.of(
            new LegendItem("Morning Shift", "bg-blue-600"),
            new LegendItem("Evening Shift", "bg-indigo-500"),
        ...
        return new SchedulingSection(
            "Managing " + (nextWeekShifts.isEmpty() ? "the upcoming rota" : nextWeekShifts.getFirst().getShiftDate().format(DATE_LABEL) + " onward") + " for pharmacy coverage.",
            stats,
            days,
            rows,
            legend,
            new SchedulingOverviewCard(
                "Schedule Overview",
                "This week's staffing snapshot",
                busiestShift == null ? "No shifts yet" : formatShiftWindow(busiestShift),
                employees.size() + " pharmacy staff active",
                openShifts + " staffing gap" + (openShifts == 1 ? "" : "s") + " this week"
            ),
            new SchedulingSuggestion(
                "Auto Suggest",
                "Balance dispensing and stock coverage",
                openShifts > 0
                    ? "Assign one additional pharmacist to the earliest understaffed shift to keep prescription intake and inventory handover uninterrupted."
                    : "Coverage is healthy. Keep the current rotation and watch closing shift fatigue later in the week.",
    ...
    private AdjustmentsSection buildAdjustmentsSection(List<ShiftAdjustmentRequest> adjustments, List<AuditLog> auditLogs) {
        List<AdjustmentRequestCard> requests = adjustments.stream()
            ...
            .map(request -> new AdjustmentRequestCard(
                ...
                request.getCreatedAt().format(DATE_LABEL),
                request.getEmployee().getFullName(),
                request.getShift().getShiftDate().format(DATE_LABEL) + " • " + formatShiftWindow(request.getShift()),
                request.getStatus() == AdjustmentStatus.PENDING ? "Manager review pending" : "Request processed",
        ...
        return new AdjustmentsSection(
            "Review live shift change requests coming from Ngabo Pharmacy staff and keep branch coverage safe.",
            requests,
            List.of(
    ...
    private NotificationsSection buildNotificationsSection(List<Notification> notifications, List<ShiftAdjustmentRequest> adjustments, List<CompliancePolicy> policies) {
        ...
        return new NotificationsSection(
            "Stay informed about staffing gaps, policy reminders, and branch communication flowing through the live manager queue.",
            "Today",
            "Earlier",
            List.of(
            ...
            "Clear urgent coverage issues first, then work through reminders and branch updates before the next handover."
        );
    }
    ...
    private ComplianceSection buildComplianceSection(List<Shift> shifts, List<CompliancePolicy> policies, List<AuditLog> auditLogs) {
        ...
        return new ComplianceSection(
            "Monitor labor rules, pharmacy operations standards, and scheduling safeguards at branch level.",
            new ComplianceAlert(
    ...
    private ReportsSection buildReportsSection(
        ...
        int attendance = shifts.isEmpty() ? 0 : (int) Math.round(
            shifts.stream().mapToInt(Shift::getAssignedStaff).sum() * 100.0 /
                Math.max(1, shifts.stream().mapToInt(Shift::getRequiredStaff).sum())
        );
        int overtimeHours = (int) assignments.stream()
            .map(ShiftAssignment::getShift)
            .mapToLong(shift -> Math.max(0, java.time.Duration.between(shift.getStartTime(), shift.getEndTime()).toHours() - 8))
            .sum();
        int capacityPercent = Math.min(100, Math.max(35, attendance));

        List<Integer> attendanceBars = List.of(78, 82, 80, 88, 91, 70, 68);
    ...
    private SettingsSection buildSettingsSection(User manager, List<CompliancePolicy> policies) {
        return new SettingsSection(
            "Configure branch rules and manager-level visibility settings for " + manager.getBranch().getName() + ".",
            List.of(
                new VisibilityRule("Show Salaries", "Only payroll-approved managers can view pay rates on shared schedules.", false),
                new VisibilityRule("Phone Numbers", "Managers and licensed pharmacy staff can view verified emergency contacts.", true),
                new VisibilityRule("Public Profiles", "Qualifications and service roles are visible across the branch team.", true)
            ),
            List.of(
                new WorkflowRule("Shift Swaps", "Manual Review", "Managers verify all peer-to-peer shift exchanges before approval.", "Configure Rules"),
                new WorkflowRule("Overtime Alerts", "Threshold: 40h", "Receive immediate alerts when a staff member exceeds the weekly cap.", null)
            ),
    ...
    private List<String> expertiseForRole(String jobTitle) {
        ...
        return List.of("Dispensing", "Patient counselling", "Controlled drug logs", "Branch operations");
    }
